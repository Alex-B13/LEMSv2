...
#define SNOW_POWER_PIN 12   //snow depth sensor power pin
...
#define GPS_POWER 11        // GPS power pin, could also use D12
...
char filename[] = "LEMStest.CSV";   // Initial filename - Whatever this is named, the filename should be 8 characters long and the last two should be digits!
...
// GPS TinyGPS++.h
TinyGPSPlus gps;
char position_filename[] = "LOCLE200.CSV";  // Initial GPS location filename - Whatever this is named, the filename should be 8 characters long and the last two should be digits!
File myfile;
...
const int numToStart = 20;  //number of locations taken during setup (must be larger than numEach)
const int numEach = 2;   //number of locations taken each time GPS triggers in loop
...
double locations[numToStart][2]; // (lat, lng)
...
double pressure;             // Barometric pressure-2
...
const double liConst = 5.7010E-5/1000;  // Licor Calibration Constant. Units of (Amps/(W/m^2))
...
//  }
  ...
  if (encode_GPS_time(30000)) { // 30 sec max wait to get GPS time
    // print time
    DateTime now = rtc.now();
  }
  digitalWrite(GPS_POWER, LOW);

get_position_points(numToStart);
 ...
 // Open SD file
  ...
  for (uint8_t i = 0; i < 100; i++) {
    position_filename[6] = '0' + i / 10;
    position_filename[7] = '0' + i % 10;
    if (! SD.exists(position_filename)) {
      break;
    }
  }
...
SerialUSB.print("time adjusted to:");
SerialUSB.print(gps.date.year());
SerialUSB.print(gps.date.month());
SerialUSB.print(gps.date.day());
SerialUSB.print(gps.time.hour());
SerialUSB.print(gps.time.minute());
SerialUSB.println(gps.time.second());
  #endif
...
//Getting snow first to not have the long delay between time and 
...
#if SNOW
  digitalWrite(SNOW_POWER_PIN, HIGH);
  delay(400);
  ...
  digitalWrite(SNOW_POWER_PIN, LOW);
#endif
#if IR
...
bool encode_GPS_position(int timeout) {
  ...
  while (1) {
    ...
    if (gpsSerial.available()) {
    #if DEBUG
      SerialUSB.println("gpsSerial.available");
    #endif
      ...
      //SerialUSB.println("Any data available");

      // Get time from GPS data
      if (gps.location.isUpdated() && gps.location.isValid() ) {


        //SerialUSB.println("Location available and updated");
        return true;
      } else {
...
bool get_position_points(int numToGet)
{
  #if DEBUG
SerialUSB.print("getting GPS Points");
  #endif
  ...
  #if DEBUG
  SerialUSB.println("Bad GPS Data");
  #endif
    }
    
  }
  #if DEBUG
  SerialUSB.println("got points");
  #endif
