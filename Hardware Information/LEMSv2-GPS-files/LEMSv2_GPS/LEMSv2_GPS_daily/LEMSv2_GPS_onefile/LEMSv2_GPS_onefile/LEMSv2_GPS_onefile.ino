...
// GPS TinyGPS++.h
TinyGPSPlus gps;
char position_filename[] = "GPSLOC00.CSV";  // Initial GPS location filename - Whatever this is named, the filename should be 8 characters long and the last two should be digits!
File myfile;
...
const int numToStart = 10;  //number of locations taken during setup (must be larger than numEach)
...
const int trigger_count = 1440; //number of cycles before the GPS takes a reading (# of 10-second cycles in a day divided by number of times to take a reading per day)
int counter = trigger_count; // variable for counting cycles since last measure, initially set so the first reading will be taken on first cycle
#endif
  ...
  if (!logfile) {
    ...
    for (int i = 0; i < numToStart; i++) {
     ...
     // Serial.println(locations[i][1], 6);
      logfile.print(locations[i][0], 6);
      logfile.print(",");
      logfile.println(locations[i][1], 6);
    }
    logfile.flush();
...
#endif
#if GPS
  logfile.print(",Latitude,Longitude");
#endif
  logfile.println();
  logfile.flush();
...
#if GPS
  if (counter == trigger_count) {
      logfile.print(locations[1][0], 6);
      logfile.print(",");
      logfile.println(locations[1][1], 6);
//
//   // Serial.println("Done writing to SD");
//  }
  counter = 0;
